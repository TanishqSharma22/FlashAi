 ...
 * This function is for free or premium users, with a usage limit for free users.
 */
export const generateArticle = async (req, res) => {
    try {
        ...
        // Check if the user is a free user and has reached their limit.
        if (plan !== 'premium' && free_usage >= 10) {
            return res.status(403).json({ success: false, message: "Limit reached. Upgrade to continue." });
        }

        // Call the AI model to generate the article content.
        ...
        // Update the free usage count for non-premium users.
        if (plan !== 'premium') {
            await clerkClient.users.updateUserMetadata(userId, {
                privateMetadata: {
                    // free_usage is guaranteed to be a number by the auth middleware
...
/**
 * Generates blog titles using the Gemini API.
 * This function is for free or premium users, with a usage limit for free users.
 */
export const generateBlogTitles = async (req, res) => {
    try {
        ...
        // Call the AI model to generate blog titles.
        const response = await AI.chat.completions.create({
            model: "gemini-2.0-flash",
            messages: [{
                role: "user",
...
/**
 * Generates an image using the ClipDrop API.
...
export const removeImageObject = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ success: false, message: "An error occurred while removing the image object." });
    }
};
...
export const resumeReview = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ success: false, message: "An error occurred during resume review." });
    }
};